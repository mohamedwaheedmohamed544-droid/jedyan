...
export default function WhyJedyan({ locale, eyebrow = "09 · WHY JEDYAN" }: { locale: Locale; eyebrow?: string }) {
  const L = locale;
  return (
      ...
      <div className="wrap">
        <div className="section-head">
          ...
          <h2 id="whyj-title" className="h2" data-reveal style={{ ["--d" as string]: 80 }}>
            {L === "ar" ? "ستة أسباب تجعل جديان شريك تشغيل يُعتمد عليه." : "Six reasons Jedyan is an operating partner you can rely on."}
          </h2>
        </div>
        <ul className={s.grid}>
          {reasons.map((r, i) => (
