...
import { visibility } from "@/content/site";
...
export default function Visibility({ locale, eyebrow = "08 · TECHNOLOGY & VISIBILITY" }: { locale: Locale; eyebrow?: string }) {
  const L = locale;
      ...
      <div className="wrap split">
        <div className={s.copy}>
          ...
          <h2 id="vis-title" className="h2" data-reveal style={{ ["--d" as string]: 80 }}>
            {L === "ar" ? "ما لا يمكن رؤيته، لا يمكن إدارته." : "What can't be seen can't be managed."}
          </h2>
          <p className="lead muted" data-reveal style={{ ["--d" as string]: 140 }}>
            {L === "ar"
              ? "نستخدم أنظمة تشغيل وتقارير تجعل حالة المخزون والطلب معلومة قبل أن تُطلب. الإفصاح الاستباقي: نقول ما نعرفه قبل أن يُسأل عنه."
              : "We use operating systems and reporting that make stock and order status known before anyone asks. Proactive disclosure: we say what we know before we're asked."}
          </p>
          <ul className={s.list}>
            {visibility.map((v, i) => (
