 ...
 */
export default function CommerceJourney({ locale, compactHead = false, b }: { locale: Locale; compactHead?: boolean; b?: Record<string, unknown> }) {
  const L = locale;
  const { journey } = useSite();
        ...
        <div className={s.sticky}>
          <div className={`wrap ${s.inner}`}>
            <header className={s.head}>
              <p className="eyebrow">{px(b, "eyebrow", L, "THE COMMERCE OPERATIONS JOURNEY")}</p>
              <h2 id="journey-title" className={compactHead ? "h3" : "h2"}>
                {px(b, "title", L, L === "ar" ? "من دخول البضاعة حتى إغلاق الطلب — رحلة تشغيل واحدة." : "From goods-in to order closure — one operating journey.")}
              </h2>
            </header>
