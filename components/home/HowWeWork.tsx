...
import { clientJourney } from "@/content/site";
import { useInView } from "@/lib/hooks";
import s from "./HowWeWork.module.css";

export default function HowWeWork({ locale, eyebrow = "07 · HOW WE WORK" }: { locale: Locale; eyebrow?: string }) {
  const L = locale;
      ...
      <div className="wrap">
        <div className="section-head">
          ...
          <h2 id="how-title" className="h2" data-reveal style={{ ["--d" as string]: 80 }}>
            {L === "ar" ? "من التشخيص إلى التوسّع — رحلة واحدة بست محطات." : "From diagnosis to scale — one journey, six stations."}
          </h2>
          <p className="lead" data-reveal style={{ ["--d" as string]: 140 }}>
            {L === "ar" ? "لا نبدأ بالسعر. نبدأ بفهم شكل الطلب، لأن التسعير الصحيح نتيجة تشخيص صحيح." : "We don't start with price. We start by understanding your orders — correct pricing is the result of a correct diagnosis."}
          </p>
        </div>
