...
export default function WhyExists({ locale, b }: { locale: Locale; b?: Record<string, unknown> }) {
  const L = locale;
  const { whyExists } = useSite();
    ...
    <section className={`section dark ${s.section}`} aria-labelledby="why-title">
      <div className="wrap">
        <div className="section-head">
          <p className="eyebrow" data-reveal>{px(b, "eyebrow", L, "03 · WHY JEDYAN EXISTS")}</p>
          <h2 id="why-title" className="h2" data-reveal style={{ ["--d" as string]: 80 }}>{px(b, "title", L, whyExists.title[L])}</h2>
        </div>
