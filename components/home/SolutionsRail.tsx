...
export default function SolutionsRail({ locale, b }: { locale: Locale; b?: Record<string, unknown> }) {
  const L = locale;
      ...
      <div ref={runway} className={desktop ? s.runway : ""} style={style}>
        <div className={desktop ? s.sticky : s.static}>
          <div className={`wrap ${s.head}`}>
            <div>
              <p className="eyebrow" data-reveal>{px(b, "eyebrow", L, "04 · OUR SOLUTIONS")}</p>
              <h2 id="sol-title" className="h2" data-reveal style={{ ["--d" as string]: 80 }}>
                {px(b, "title", L, L === "ar" ? "ستة محاور تبني رحلة واحدة." : "Six pillars, one journey.")}
              </h2>
              <p className="lead muted" data-reveal style={{ ["--d" as string]: 140 }}>
                {px(b, "lead", L, L === "ar" ? "العميل لا يشتري ستة عقود — يشتري نتيجة واحدة مصمّمة من ستة محاور." : "Clients buy one outcome designed from six pillars.")}
              </p>
            </div>
            <div className={s.counter} aria-hidden="true">
              ...
              <span className="mono">{String(solutions.length).padStart(2, "0")}</span>
            </div>
          </div>
