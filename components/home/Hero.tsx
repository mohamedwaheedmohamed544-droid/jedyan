...
import { journey, messages } from "@/content/site";
    ...
    <section className={`dark ${s.hero}`} aria-labelledby="hero-title">
      ...
      <div className={`wrap ${s.inner}`}>
        <div className={s.copy}>
          <p className="eyebrow" data-reveal>COMMERCE OPERATIONS · KSA</p>
          <h1 id="hero-title" className={`lines ${s.title}`}>
            <span style={{ ["--i" as string]: 0 }}><span>{messages.bigIdea1[L]}</span></span>
            <span style={{ ["--i" as string]: 1 }}><span className="accent">{messages.bigIdea2[L]}</span></span>
          </h1>
          <p className={s.support} data-reveal style={{ ["--d" as string]: 350 }}>{messages.heroSupport[L]}</p>
          <div className={s.ctas} data-reveal style={{ ["--d" as string]: 500 }}>
            <Magnetic>
              <Link href={localePath(L, "/contact")} className="btn">{ui("requestModel", L)} <Arrow /></Link>
            </Magnetic>
            <Link href={localePath(L, "/how-we-work")} className={`btn btn--ghost ${s.ghost}`}>{L === "ar" ? "شاهد كيف نعمل" : "See how we work"}</Link>
          </div>
        </div>
