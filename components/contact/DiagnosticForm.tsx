...
const ENDPOINT = process.env.NEXT_PUBLIC_LEAD_ENDPOINT; // optional: POST JSON here (CRM / form service)
...
export default function DiagnosticForm({ locale }: { locale: Locale }) {
  ...
  const submit = async (ev: React.FormEvent) => {
    ...
    if (!validate()) return;
    setState("sending");
    const payload = { source: "jedyan.sa/contact", locale: L, contact, answers: summary };
    if (ENDPOINT) {
      try {
  ...
  if (state === "done") {
    return (
      <div className={s.card} role="status">
        ...
        {!ENDPOINT && <p className="caption muted">{t("إذا لم يُفتح بريدك تلقائيًا، راسلنا على", "If your email app didn't open, write to")} <a href={`mailto:${company.email}`} className="accent latin">{company.email}</a></p>}
      </div>
    );
  }
